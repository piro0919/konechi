#!/bin/bash
# Konechi を1つ上げる。DMG を作り、更新情報に署名して、GitHub Releases に置く。
#
#   ./release.sh 1.0.1
#
...
REPO="piro0919/konechi"
APP="Konechi.app"
DMG="Konechi-${VERSION}.dmg"
ZIP="Konechi-${VERSION}.zip"

# 版数を Info.plist に焼き込むため、build.sh へ渡す
KONECHI_VERSION="$VERSION" ./build.sh
...
hdiutil create -quiet -volname "Konechi" -srcfolder "$STAGE" -ov -format UDZO "dist/$DMG"
rm -rf "$STAGE"
